...
macros.return = (function(token) {
  ...
  return ((token) && (token).constructor.name === "Array" ? (function() {
    ...
    case "return":
    case "throw":
    case "do":return translate(token);
    case "delete":var deleteMacro = (macros)["delete"];; return ((token.length < 3) ? defaultReturn : (deleteMacro.apply(undefined, token.slice(1, -1)) + "\nreturn " + deleteMacro((token.slice(-1))[0])));
    case "=":return ((token.length < 4) ? defaultReturn : ((macros)["="].apply(undefined, token.slice(1, (token.length - 2))) + "\nreturn " + (macros)["="].apply(undefined, token.slice(-2))));
    case "set":return ((token.length < 5) ? defaultReturn : (function() { var obj = (token)[1],
